...
| `gateway.securityMode`      | Gateway security mode, GATEWAY_MANAGED or KAFKA_MANAGED. Used only when gateway.preview.listeners is true. Emitted as GATEWAY_SECURITY_MODE (gateway.env.GATEWAY_SECURITY_MODE takes precedence if set).                 | `GATEWAY_MANAGED` |
| `gateway.aclEnabled`        | Enable ACL for the Gateway virtual cluster. Used only when gateway.preview.listeners is true. Emitted as GATEWAY_ACL_ENABLED. Inferred from securityMode when empty (true for GATEWAY_MANAGED, false for KAFKA_MANAGED). | `""`              |
