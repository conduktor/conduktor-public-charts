...
| `gateway.image.tag`        | Image tag                                                | `3.21.0`                      |
...
| `tls.keystore.keyPasswordSecretRef.name`   | Secret containing the private key password inside the JKS, when it differs from the keystore password. Leave empty to reuse tls.keystore.passwordSecretRef. Not supported with tls.certManager.enabled, which issues keystores whose key password always matches the store password               | `""`             |
| `tls.keystore.keyPasswordSecretRef.key`    | Key in the private key password secret                                                                                                                                                                                                                                                            | `password`       |
...
| `metrics.lifecycle.enable`               | Enable the request lifecycle metrics feature. Emits GATEWAY_FEATURE_FLAGS_LIFECYCLE_METRICS=true so Gateway records the per-stage request lifecycle histograms (high cardinality, off by default). Independent of metrics.grafana.enable; when metrics.grafana.enable is also true, the request lifecycle Grafana dashboards are deployed as well. | `false`                      |
| `metrics.lifecycle.apiKeys`              | Scope the lifecycle metrics to a comma-separated list of Kafka ApiKeys names (e.g. "PRODUCE,FETCH"), or "ALL" (default) for every ApiKey. Emitted as GATEWAY_LIFECYCLE_METRICS_API_KEYS only when set to a value other than "ALL". Only used when metrics.lifecycle.enable is true.                                                                | `ALL`                        |
| `metrics.lifecycle.interceptors`         | Scope the per-interceptor lifecycle metrics to a comma-separated list of interceptor logical names, or "ALL" (default) for every interceptor. Emitted as GATEWAY_LIFECYCLE_METRICS_INTERCEPTORS only when set to a value other than "ALL". Only used when metrics.lifecycle.enable is true.                                                        | `ALL`                        |
...
##### Request lifecycle metrics and dashboards
The chart can enable the **request lifecycle** metrics feature, which makes Gateway record per-stage
latency histograms (preprocessing, authorization, interceptors, the Kafka round-trip, response send, etc)
by emitting `GATEWAY_FEATURE_FLAGS_LIFECYCLE_METRICS=true`. These metrics are high cardinality and are
off by default.
...
metrics:
  lifecycle:
    enable: true
    # Optional cardinality scoping — default "ALL" for both.
    # apiKeys: "PRODUCE,FETCH"
    # interceptors: "encrypt,decrypt"
```

`metrics.lifecycle.enable` drives the feature flag on its own, independently of `metrics.grafana.enable`.
To bound cardinality, `metrics.lifecycle.apiKeys` and `metrics.lifecycle.interceptors` scope which Kafka
ApiKeys and interceptors are recorded; each accepts a comma-separated list or the sentinel `ALL` (default),
and is emitted as `GATEWAY_LIFECYCLE_METRICS_API_KEYS` / `GATEWAY_LIFECYCLE_METRICS_INTERCEPTORS`
respectively only when set to a value other than `ALL`.

When both `metrics.grafana.enable` and `metrics.lifecycle.enable` are `true`, the chart also deploys 3
additional request lifecycle Grafana dashboards alongside the main dashboard.
