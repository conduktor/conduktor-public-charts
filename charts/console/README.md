...
| `platform.image.tag`                             | Conduktor Console image tag (immutable tags are recommended)                                                                                                                                                                     | `1.46.2`                      |
...
| `platform.debugSidecar.enabled`                  | Deploy the Conduktor debug image as a sidecar of the Conduktor Console container                                                                                                                                                 | `false`                       |
| `platform.debugSidecar.shareProcessNamespace`    | Share the pod process namespace so the JVM tools (`jcmd`, `jstack`, `jmap`, `jfr`) can attach to the Console process                                                                                                             | `true`                        |
| `platform.debugSidecar.image.registry`           | Conduktor debug image registry                                                                                                                                                                                                   | `docker.io`                   |
| `platform.debugSidecar.image.repository`         | Conduktor debug image repository                                                                                                                                                                                                 | `conduktor/conduktor-debug`   |
| `platform.debugSidecar.image.tag`                | Conduktor debug image tag (immutable date tags such as `2026.08.12` are recommended)                                                                                                                                             | `latest`                      |
| `platform.debugSidecar.image.digest`             | Conduktor debug image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag image tag                                                                                                        | `""`                          |
| `platform.debugSidecar.image.pullPolicy`         | Conduktor debug image pull policy                                                                                                                                                                                                | `IfNotPresent`                |
| `platform.debugSidecar.image.pullSecrets`        | Conduktor debug image pull secrets                                                                                                                                                                                               | `[]`                          |
| `platform.debugSidecar.command`                  | Command to keep the debug sidecar alive                                                                                                                                                                                          | `["sleep","infinity"]`        |
| `platform.debugSidecar.args`                     | Args for the debug sidecar container                                                                                                                                                                                             | `[]`                          |
| `platform.debugSidecar.containerSecurityContext` | Conduktor debug sidecar container Security Context. Defaults to the Console UID/GID (`10001`), which is what the JVM attach mechanism requires. Add the `NET_RAW` capability for `tcpdump`, or `SYS_PTRACE` for `strace`/`jhsdb` | `{}`                          |
| `platform.debugSidecar.resources`                | Conduktor debug sidecar container resource requests and limits                                                                                                                                                                   | `{}`                          |
| `platform.debugSidecar.extraEnvVars`             | Array with extra environment variables to add to the debug sidecar container                                                                                                                                                     | `[]`                          |
| `platform.debugSidecar.extraVolumeMounts`        | Extra list of additional volumeMounts for the debug sidecar container. Volumes are declared in `platform.extraVolumes`                                                                                                           | `[]`                          |
...
| `platformCortex.image.tag`                             | Conduktor Console Cortex image tag (immutable tags are recommended)                                                                                                                            | `1.46.2`                             |
...
| `platformCortex.debugSidecar.enabled`                  | Deploy the Conduktor debug image as a sidecar of the Conduktor Console Cortex container                                                                                                        | `false`                              |
| `platformCortex.debugSidecar.shareProcessNamespace`    | Share the pod process namespace. Disabled by default: unlike Console, Cortex is not a JVM, so there is no process to attach to. Enable it to let `strace` and `ss -p` see the Cortex processes | `false`                              |
| `platformCortex.debugSidecar.image.registry`           | Conduktor debug image registry                                                                                                                                                                 | `docker.io`                          |
| `platformCortex.debugSidecar.image.repository`         | Conduktor debug image repository                                                                                                                                                               | `conduktor/conduktor-debug`          |
| `platformCortex.debugSidecar.image.tag`                | Conduktor debug image tag (immutable date tags such as `2026.08.12` are recommended)                                                                                                           | `latest`                             |
| `platformCortex.debugSidecar.image.digest`             | Conduktor debug image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag image tag                                                                      | `""`                                 |
| `platformCortex.debugSidecar.image.pullPolicy`         | Conduktor debug image pull policy                                                                                                                                                              | `IfNotPresent`                       |
| `platformCortex.debugSidecar.image.pullSecrets`        | Conduktor debug image pull secrets                                                                                                                                                             | `[]`                                 |
| `platformCortex.debugSidecar.command`                  | Command to keep the debug sidecar alive                                                                                                                                                        | `["sleep","infinity"]`               |
| `platformCortex.debugSidecar.args`                     | Args for the debug sidecar container                                                                                                                                                           | `[]`                                 |
| `platformCortex.debugSidecar.containerSecurityContext` | Conduktor debug sidecar container Security Context. Defaults to the Cortex UID/GID (`10001`). Add the `NET_RAW` capability for `tcpdump`, or `SYS_PTRACE` for `strace`                         | `{}`                                 |
| `platformCortex.debugSidecar.resources`                | Conduktor debug sidecar container resource requests and limits                                                                                                                                 | `{}`                                 |
| `platformCortex.debugSidecar.extraEnvVars`             | Array with extra environment variables to add to the debug sidecar container                                                                                                                   | `[]`                                 |
| `platformCortex.debugSidecar.extraVolumeMounts`        | Extra list of additional volumeMounts for the debug sidecar container. Volumes are declared in `platformCortex.extraVolumes`                                                                   | `[]`                                 |
...
### Install the Conduktor debug sidecar

[`conduktor/conduktor-debug`](https://hub.docker.com/r/conduktor/conduktor-debug) is our support toolbox image: JVM diagnostics (`jcmd`, `jstack`, `jmap`, `jfr`, `async-profiler`), network (`tcpdump`, `ss`, `dig`, `mtr`, `nc`), TLS (`openssl`, `certutil`), LDAP (`ldapsearch`), Kafka (`kcat`, the Apache `kafka-*.sh` tools), PostgreSQL (`psql`, `pgcli`), the `conduktor` CLI, plus `cdk-*` helpers that read everything from the Console process itself and redact secrets by default.

Enable it as a sidecar of the Console container:
...
This adds a `debug` container to the Console pod and sets `shareProcessNamespace: true` so the JVM tools can attach to the Console process. The sidecar runs as UID/GID `10001` — the same as the Console container — which is what the JVM attach mechanism requires; `SYS_PTRACE` is *not* needed for it.

Then get a shell and run the triage helpers:

```bash
kubectl exec -it $CONSOLE_POD_NAME -n conduktor -c debug -- bash

cdk-doctor          # one-shot triage, exit code = number of failures
cdk-target          # visible JVMs, their UID and derived config paths
cdk-config          # the rendered per-app configuration
cdk-jvm threads     # thread dump over the attach mechanism
cdk-pg check        # is Console's PostgreSQL reachable and healthy?
cdk-tls kafka:9093  # certificate chain validated against Console's own truststore
```

Some tools need extra privileges. Restate the container security context to grant them — `platform.debugSidecar.containerSecurityContext` replaces the chart default, so repeat the UID/GID:
...
platform:
  debugSidecar:
    ...
    containerSecurityContext:
      runAsNonRoot: true
      runAsUser: 10001    # must match the Console container UID
      runAsGroup: 10001
...
> [!NOTE]
> The image is ~330 MB compressed (it ships a full JDK). If your cluster is air-gapped, mirror it alongside the Console image as described in [Install in an air-gapped environment](#install-in-an-air-gapped-environment), and set `platform.debugSidecar.image.registry` / `repository` accordingly.

> [!IMPORTANT]
> A shared process namespace lets both containers of the pod see each other's processes and `/proc`, so keep the sidecar enabled only while you need it. Set `platform.debugSidecar.shareProcessNamespace: false` to keep the network, TLS, Kafka and PostgreSQL tooling while giving up JVM attach.

#### On the Cortex pod

The same sidecar is available on the monitoring pod under `platformCortex.debugSidecar.*`, with the same parameters:
...
Enable it there when the *monitoring* stack is the problem — object storage that Cortex cannot reach, a TLS handshake against your S3 endpoint, DNS, or the Cortex ⇄ Console callback URLs:

```bash
kubectl exec -it $CORTEX_POD_NAME -n conduktor -c debug -- bash

curl -sv https://my-s3-endpoint/                    # object storage reachability + TLS
openssl s_client -connect my-s3-endpoint:443        # certificate chain
dig +short my-s3-endpoint
curl -s localhost:9009/ready                        # Cortex readiness, from inside the pod network
```

The two blocks are enabled independently — the image is the same, so there is no reason to carry it in both pods at once. Two differences from the Console pod:

- `platformCortex.debugSidecar.shareProcessNamespace` defaults to **`false`**: Cortex is a Go binary, not a JVM, so there is no process to attach to and nothing to gain from weakening pod isolation. Set it to `true` if you want `strace` or `ss -p` to resolve the Cortex processes.
- The JVM-oriented helpers (`cdk-target`, `cdk-jvm`, `cdk-config`) find no JVM in that pod. The network, TLS and PostgreSQL tooling all work normally.
...
```bash
# --target is the container name: conduktor-platform, or conduktor-platform-cortex on the monitoring pod
kubectl debug -n conduktor $CONSOLE_POD_NAME \
  --image=conduktor/conduktor-debug:latest \
  --target=conduktor-platform --profile=general -it -- bash
```
